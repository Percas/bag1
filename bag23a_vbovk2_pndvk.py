...
subdoel1: we willen alles op vbovk niveau berekenen. Daarvoor is het het
beste als elk vbo binnen de looptijd van al zijn pndvk valt. Dit hoeft niet zo 
te zijn. Daarom maken we extra vbovk aan:
...
def bag_vbovk_pndvk(current_month='testdata',
    ...
    print('\t\t2.1 Neem de unieke vbo-pnd uit vbo df...')
    vbo_df = bd['vbo'][['vboid', 'pndid']].drop_duplicates()
    
       
    print('\n\t\t2.2 Voeg aan de vbo de pndvkbg (vk begindatum) toe van pnd df...')
    print('\t\t\tEenheid id is hier vbo + pndvkbg!')
    vbo_df = pd.merge(vbo_df,
                      bd['pnd'][['pndid', 'pndvkbg']])[['vboid', 'pndvkbg']]
