...
import time
import baglib
from config import OMGEVING, DIR02, DIR03, FUTURE_DATE, BAG_TYPE_DICT
...
def bag_fix_vk(loglevel = 10,
    ...
    for bob in relevant_cols.keys():
        ...
        bd[bob] = baglib.merge_vk(ll, bd[bob], bob, FUTURE_DATE, relevant_cols[bob])
...
def vksplitter(loglevel=10,
 ...
 '''
   ...
   # #############################################################################
    ...
    _df = baglib.make_vkeg(ll, _df, fijntype, FUTURE_DATE)
